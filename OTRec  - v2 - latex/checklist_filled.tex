...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Code, preprocessed data, model weights, and ranked predictions are released under MIT at the repository linked in the Availability section; OTP source data is publicly available under CC0. Link will be shared after review.
...
\item {\bf Licenses for existing assets}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Open Targets (CC0), EFO (Apache 2.0), gnomAD (ODbL), Reactome (CC-BY 4.0), UniProt (CC-BY 4.0), TensorFlow/Keras (Apache 2.0), CatBoost (Apache 2.0), and XGBoost (Apache 2.0) are all cited; the released code, results and repo use MIT license (link to repo anonymized during review period).
...
\item {\bf Declaration of LLM usage}
    ...
    \item[] Answer: \answerNA{}
    \item[] Justification: No LLM is part of the OTRec method; LLMs were used only for incidental writing/editing assistance, which the NeurIPS policy does not require declaring. The Interfeat tool was used for helping select candidates prior to author's own selection and review.
