...
Validated in a temporal hold-out against later trial entries, it improves on the retrospective OTP score and the learned baselines in the target-disjoint CV. We release candidate associations ${\sim}282{,}500$ candidate associations above a $0.65$ score threshold with a fully interactive platform.
...
The OTP known-drug exports used here do not a consistently available trial-start-date field, so we do not independently censor by ClinicalTrials.gov start date. The resulting test set covers $11{,}624$ diseases
...
(\textit{ix})~\textbf{Frozen biomedical encoder}: normalized
BioClinical-ModernBERT-base disease and target embeddings
(\texttt{thomas-sounack/BioClinical-ModernBERT-base}) scored with a
small MLP interaction head over concatenation, absolute difference,
elementwise product, and cosine similarity. Early stopping used a held-out 10\% ; test targets are never seen
during training.
...
A frozen BioClinical-ModernBERT-base encoder reached
$0.912 \pm 0.008$ ROC-AUC and $0.666 \pm 0.028$ PR-AUC ($5{\times}5$ CV, same protocol) --- above the simple priors, but below the best models in PR-AUC.
...
The BioClinical-ModernBERT encoder, achieves $0.856 \pm 0.003$ ROC-AUC and
...
responsibility. Other limitations concern several of the technical decisions along with the OTRec implementation. Specifically, the auxiliary head regresses the OTP overall association score from the
...
supervision. The reported temporal mean performance uses a multi-task
...
interpretability of contributing tokens. In a separate frozen
BioClinical-ModernBERT-base baseline, pretrained embeddings improved
substantially over TF-IDF but remained below OTRec and OTTree in
target-disjoint CV, so stronger end-to-end encoder adaptation remains
an open direction rather than an established win here.
...
During blind review, an anonymized supplementary contains the code
needed to reproduce the main analyses and the result CSVs used in the
manuscript. The public repository, models, ranked predictions, and
interactive query interface are online but not linked during anonymized
review; they will be shared publicly after acceptance / camera-ready.
