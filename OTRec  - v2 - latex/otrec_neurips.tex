...
score  (ROC-AUC: $0.872 \pm 0.005$ vs.\ $0.559$; PR-AUC: $0.288 \pm 0.009$ vs.\ $0.08$). %) it remains close to a target-prior baseline (, gap $0.011$).
In $5{\times}5$ target-disjoint cross-validation, OTRec reaches
ROC-AUC $0.950$ and PR-AUC $0.844$, improving on the Open-Targets evidence score (ROC-AUC $0.91$; PR-AUC $0.45$).
...
The public demo, code, models, and predictions will be released after the review period.
\end{abstract}
...
No special censoring is applied for diseases or targets with
...
Training in TensorFlow/Keras~\citep{chollet2015} used Adam (learning rate $8{\times}10^{-3}$, batch size $1{,}024$), BinaryCrossentropy loss, ReduceLROnPlateau (factor $0.2$, patience $1$) and early stopping (patience $2$) on a held-out 5\% validation set.
...
(\textit{ix})~\textbf{Frozen BioClinical ModernBERT + MLP}~\citep{sounack2025bioclinicalmodernbert}: static disease and target embeddings from BioClinical ModernBERT scored with a small MLP head over concatenation, absolute difference, elementwise product, and cosine similarity; test targets are unseen during training.
...
\begin{figure}[t]
  ...
  \caption{
    ...
    \textit{[Architecture diagram to be added in camera-ready version]}
    \textbf{(B)} Target-disjoint $5{\times}5$ cross-validation results (mean $\pm$ SD over 25 folds).
    OTRec achieves the highest mean ROC-AUC ($0.950 \pm 0.007$) and
    PR-AUC ($0.844 \pm 0.017$).
    \textbf{(C)} Temporal hold-out validation (training on 2022,
    predicting 2025 entries). Mean and standard deviation over five random initialization seeds. Error bars denote standard deviation.
  }
  \label{fig:model}
\end{figure}
...
achieves ROC-AUC $0.950$ (PR-AUC $0.844$). Compared to OTTree ($0.947$ / $0.772$),
OTRec achieves similar ROC-AUC but substantially higher PR-AUC (gain of $0.072$).
Both substantially exceed the Open Targets Score ($0.914$ / $0.455$).
...
OTRec's absolute gain over Han et al.~on ROC-AUC ($+0.040$) and PR-AUC ($+0.118$) reflects both feature engineering and model class.
The OTRec--OTTree gap isolates the two-tower architecture contribution on
matched inputs: $+0.003$ ROC-AUC but $+0.072$ PR-AUC, demonstrating that
the two-tower approach particularly improves precision-recall performance.
...
\begin{figure}[t]
  ...
  candidate list with OTP/EFO cross-links, tractability
  category, and export of the full druggable-genome ranking.}
  \label{fig:gradio}
\end{figure}
...
OTRec ranked IL6R highly; IL-6R inhibitors (e.g.,  Tocilizumab, Sarilumab) are approved for Rheumatoid arthritis and cytokine release syndrome, but have shown only modest efficacy in Ulcerative colitis (UC). While biologically plausible, their clinical benefit in UC remains limited. They may therefore be better suited for patients who become resistant to standard therapies (i.e. anti-TNFs, JAK inhibitors, integrin blockers). We suggest repurposing well-characterized, safe IL-6R inhibitors for stratified patient subsets, particularly non-responders, as a second-line or complementary treatment option. % recent clinical trials look relatively negative for it. may be a bad example
